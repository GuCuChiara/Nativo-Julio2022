...
def buscar(request):
    if request.GET ["diag"]:
        ...
        codigo= cie10nativo.objects.filter(codigossancors__Presta_Id__icontains = producto) 
...
def buscar_AMS(request):			
    if request.GET ["diag4"]:
        ...
        codigo2= cie10nativo.objects.filter(codigossancors__Descripcion__icontains=producto).exclude(CIE10_DESC__icontains='Otras') #.filter(id__gt=10000) 
        #nuevo 
...
def buscar_CIE10(request): 
    if request.GET ["diag2"]:
        ...
        prod=cie10nativo.objects.filter(CIE_10_Id__icontains=producto)
        # contexto:     "producto" = lo buscado
...
def buscar_CIE10_x(request):
    if request.GET ["diag3"]:
        ...
        #----------------------------------------------------------------------------------------
        #   para ordenar según score del id del 1 al 9 en forma jerárquica
        #   codigo= cie10nativo.objects.filter(CIE10_DESC__icontains = producto).order_by('id').values() 
        #----------------------------------------------------------------------------------------
        codigo= cie10nativo.objects.filter(CIE10_DESC__icontains = producto).order_by('CIE10_DESC').values()
